...
interface Test<EntityType extends Entity["type"]> {
    
}

export function dbToEntity<EntityType extends Entity["type"], DbEntity extends Tables<'entities'> & { type: EntityType } & {[P in EntityType]: Tables<EntityType>}, TEntity extends Tables<'entities'> & { type: EntityType } & Tables<EntityType>>(db: DbEntity): TEntity {
...
export function generateTableRows(entity: Entity): Row[] {
    ...
    if (entity.type === 'spells') {
        ...
        if (entity.damage_type) {
            const die = entity.damage_at_character_level ? getFirstValue(entity.damage_at_character_level as Record<string, string>) : getFirstValue(entity.damage_at_slot_level as Record<string, string>);
            rows.push({ name: 'Damage', value: `${die} ${humanizeId(entity.damage_type)}`, popup: 'higherLevel' });
        }
...
function spellHigherLevelPopup(entity: Spell): Popup | undefined {
    ...
    const obj = (entity.damage_at_character_level || entity.damage_at_slot_level || entity.heal_at_slot_level) as Record<string, string>;
    if (obj) {
